...
void set_transmit_flag(void) { transmittedFlag = true; }

// receive
int receivedState = RADIOLIB_ERR_NONE;
volatile bool receivedFlag = false;

void set_receive_flag(void) { receivedFlag = true; }
...
uint16_t refr_times = 2; // Reduced from 4 for longevity (Eco-Clear)
...
void eeprom_default_val(void) {
  ...
  for (int i = WIFI_PSWD_EEPROM_ADDR;
       i < WIFI_PSWD_EEPROM_ADDR + WIFI_PSWD_MAX_LEN; i++) {
...
void eeprom_wr_wifi(const char *ssid, uint16_t ssid_len, const char *pwsd,
                    uint16_t pwsd_len) {
...
void eeprom_init() {
  if (!EEPROM.begin(EEPROM_SIZE_MAX)) {
    Serial.println("[eeprom] failed to initialise EEPROM");
    delay(1000000);
  }
...
void lv_port_disp_init(void) {
  ...
  lv_color_t *lv_disp_buf_p =
      (lv_color_t *)ps_calloc(sizeof(lv_color_t), DISP_BUF_SIZE);
...
void home_btn_event(void *user_data) { home_btn_st = true; }
...
void setup() {
  ...
  if (touch.begin(Wire, 0x5D, BOARD_SDA, BOARD_SCL)) {
    ...
    Serial.println("❌ TOUCH INIT FAILED! (Even though 0x5D was found)");
  }
  ...
  if (Wire.endTransmission() == 0) {
    ...
    // The premise is that Limit Pin is disabled, or it will only follow the
    // maximum charging current set by Limi tPin. Set the charging current ,
